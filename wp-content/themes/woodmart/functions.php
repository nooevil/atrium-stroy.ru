...
if( ! function_exists( 'woodmart_get_fonts_url') ) {
	function woodmart_get_fonts_url( $fonts ) {
        ...
        $font_url = add_query_arg( 'family', urlencode( $fonts ), "//fonts.googleapis.com/css&display=swap" );
...
// Удаление диапазона цены при выборе вариации
...
#
# Добавляем возможность выбирать категорию для соответсувующих товаров.
#

// Edit term page

add_action('init', function () {
    register_term_meta('product_cat', 'neighboring_category', [] );
});
...
function custom_product_taxonomy_edit_meta_field($term) {
	...
	$term_meta = get_term_meta( $t_id, "neighboring_category" )[0]; ?>
    <tr class="form-field">
    <th scope="row" valign="top"><label for="neighboring_category"><?php _e( 'Соседняя категория', 'neighboring_category_domain' ); ?></label></th>
        <td>
			<select name="neighboring_category" id="neighboring_category">
			<?foreach($cats as $cat){
				$add_selected = (esc_attr($term_meta) == $cat->term_id) ?  "selected" : "";
				?> 
				<option value="<?echo $cat->term_id?>" <?echo $add_selected?>><?echo $cat->name?> </option>
			<?}?>
			</select>
            <!-- <input type="text" name="" value="<?php //echo esc_attr( $term_meta['neighboring_category'] ) ? esc_attr( $term_meta['neighboring_category'] ) : ''; ?>"> -->
            <p class="description"><?php _e( 'Выберете соседнюю категорию','neighboring_category_domain' ); ?></p>
        </td>
    </tr>
<?php
}
...
function save_taxonomy_custom_meta( $term_id ) {
    if ( isset($_POST['neighboring_category'] ) ) {
        $term_meta = get_term_meta($term_id, "neighboring_category" );
        if ( isset ( $_POST['neighboring_category']) ) {
            $term_meta = $_POST['neighboring_category'];
        }
		// Save the option array.
        update_term_meta($term_id, "neighboring_category", $term_meta );
    }
}  
